...
def format_shots(shots):
    """将列表格式化为所需的字符串形式"""
    formatted_shots = [f"maven:{shot}" if not shot.startswith("maven:") else shot for shot in shots]
...
def query(vuln, k, shot_key):
    ...
    prompt = (
        f'Below is a Java vulnerability description. Please identify the software name affected by it. '
...
if __name__ == "__main__":
    ...
    all_prompts_json = prepare_prompts(train)
